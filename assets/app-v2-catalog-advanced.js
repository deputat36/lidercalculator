(function(){
  ...
  function rules(item){return item&&item.settings&&item.settings.pricing_rules?item.settings.pricing_rules:{}}
  function css(){
    if(e('catalogAdvancedCss'))return;
    var s=document.createElement('style');
    s.id='catalogAdvancedCss';
    s.textContent='.catalog-advanced-box{display:none;margin-top:10px;padding:10px;border:1px dashed var(--line);border-radius:12px;background:#fff}.catalog-advanced-box.show{display:block}.catalog-advanced-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.catalog-advanced-grid label{font-size:12px;font-weight:800;color:#374151}.catalog-advanced-grid input,.catalog-advanced-grid select{margin-top:4px}.catalog-advanced-hint{margin-top:8px;color:var(--muted);font-size:12px;line-height:1.4}.catalog-price-note{margin-top:8px;padding:8px 10px;border-radius:10px;background:#f0fdf4;color:#166534;font-size:12px;line-height:1.35}@media(max-width:900px){.catalog-advanced-grid{grid-template-columns:1fr}}';
    document.head.appendChild(s);
  }
  function after(ref){var box=e('catalogAdvancedBox');if(box)return box;var area=e('catalogAreaBox')||ref;box=document.createElement('div');box.id='catalogAdvancedBox';box.className='catalog-advanced-box';box.innerHTML='<div id="catalogAdvancedInner"></div><div id="catalogAdvancedHint" class="catalog-advanced-hint"></div><div id="catalogPriceNote" class="catalog-price-note" style="display:none"></div>';area.insertAdjacentElement('afterend',box);return box}
  ...
  function render(item){
    ...
    var inner=e('catalogAdvancedInner'),hint=e('catalogAdvancedHint'),priceNote=e('catalogPriceNote');
    if(!item){box.classList.remove('show');inner.innerHTML='';return}
    var m=mode(item),s=item.settings||{};
    current=item;
    lastPrice=n(e('itemClient')&&e('itemClient').value)||n(item.default_client_price)||n(item.min_client_price);
    ...
    if(m==='sign_piece'){
      ...
      hint.textContent='Размер и материал будут добавлены в комментарий позиции и повлияют на цену.';
    } else if(m==='mounting'){
      ...
      hint.textContent='Сложность, высота и выезд могут увеличить цену монтажа.';
    } else if(m==='design'){
      ...
      hint.textContent='Сложность, дополнительные варианты и срочность автоматически корректируют цену.';
    } else if(m==='placement'){
      ...
      hint.textContent='Площадка и подготовка контента учитываются в цене.';
    } else {box.classList.remove('show')}
    bindInputs();
  }
  function data(){
    var item=current||{};var m=mode(item);var d={calc_ui:m};
    if(m==='sign_piece'){d.length=n(e('advLength')&&e('advLength').value);d.width=n(e('advWidth')&&e('advWidth').value);d.material=e('advMaterial')&&e('advMaterial').value||'';d.area=Number((d.length*d.width).toFixed(4))}
  ...
  function calcPrice(){
    ...
    var m=mode(current),r=rules(current),d=data();
    var base=n(current.default_client_price)||n(current.min_client_price)||n(e('itemClient')&&e('itemClient').value)||0;
    var price=base, explain=[];
    if(m==='sign_piece'){
      var area=Math.max(n(d.area),n(r.base_area_m2)||0.06);
      var baseArea=n(r.base_area_m2)||0.06;
      var mult=(r.material_multiplier&&r.material_multiplier[d.material])?n(r.material_multiplier[d.material]):1;
      price=base*(area/baseArea)*mult;
      price=Math.max(price,n(current.min_client_price)||0);
      explain.push('площадь '+area+' м²');explain.push('материал ×'+mult);
    }
    if(m==='mounting'){
      var cm=(r.complexity_multiplier&&r.complexity_multiplier[d.complexity])?n(r.complexity_multiplier[d.complexity]):1;
      price=base*cm;
      if(d.height>(n(r.height_threshold_m)||3)){price+=n(r.height_surcharge)||0;explain.push('доплата за высоту')}
      if(d.travel){price+=n(r.travel_surcharge)||0;explain.push('выезд')}
      price=Math.max(price,n(current.min_client_price)||0);
      explain.push('сложность ×'+cm);
    }
    if(m==='design'){
      var dm=(r.complexity_multiplier&&r.complexity_multiplier[d.complexity])?n(r.complexity_multiplier[d.complexity]):1;
      var variants=Math.max(d.variants||1,1);
      var extra=1+Math.max(variants-1,0)*(n(r.extra_variant_multiplier)||0.3);
      var urgent=d.urgent==='да'?(n(r.urgent_multiplier)||1.5):1;
      price=base*dm*extra*urgent;
      price=Math.max(price,n(current.min_client_price)||0);
      explain.push('сложность ×'+dm);if(variants>1)explain.push('варианты ×'+extra.toFixed(2));if(urgent>1)explain.push('срочность ×'+urgent);
    }
    if(m==='placement'){
      var pm=(r.platform_multiplier&&r.platform_multiplier[d.platform])?n(r.platform_multiplier[d.platform]):1;
      var cp=(r.content_price&&r.content_price[d.content])?n(r.content_price[d.content]):0;
      price=base*pm+cp;
      price=Math.max(price,n(current.min_client_price)||0);
      explain.push('площадка ×'+pm);if(cp>0)explain.push('контент +'+money(cp));
    }
    return {price:Math.round(price),explain:explain};
  }
  function applyPrice(){
    var p=calcPrice();if(!p)return;
    if(e('itemClient'))e('itemClient').value=p.price;
    var note=e('catalogPriceNote');
    if(note){note.style.display='block';note.innerHTML='Рекомендованная цена клиенту: <b>'+money(p.price)+'</b>'+(p.explain.length?' • '+h(p.explain.join(', ')):'')}
  }
  ...
  function saveToLastRow(){var s=st();if(!s||!Array.isArray(s.rows)||!s.rows.length||!current)return;var row=s.rows[s.rows.length-1];row.data=Object.assign({},row.data||{},data(),{recommended_client_price:n(e('itemClient')&&e('itemClient').value)});try{if(window.LeaderV2CalcEditor&&window.LeaderV2CalcEditor.render)window.LeaderV2CalcEditor.render()}catch(x){}}
