...
function card(row) {
  ...
  return `<article class="v4-audit-card ${cls}"><span class="v4-audit-badge ${cls}">${esc(resultRu(row.result))}</span><h3>${esc(dateRu(row.created_at))}</h3><small>Причина: ${esc(row.reason || '—')}</small><small>Телефон: ${esc(row.phone_normalized || '—')} · request_id: ${esc(short(row.request_id, 44) || '—')}</small><small>Страница: ${esc(short(row.source_page_path || row.page_url || '—', 110))}</small><small>UTM: ${esc(short(utm, 110))}</small><small>User-Agent: ${esc(short(row.user_agent || '—', 120))}</small></article>`;
}
function render() {
