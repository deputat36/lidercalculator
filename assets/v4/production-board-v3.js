...
let busy = false;
let loaded = false;
let state = { production: [], installation: [], orders: new Map() };
...
const productionColumns = [
  ['Не передано', ['не передано', 'макет не проверен', 'макет не начат']],
  ['В работе', ['передано', 'в работе', 'печать', 'резка', 'сборка']],
  ['Готово', ['готов']],
  ['Проблемы', ['проблем', 'срыв', 'передел', 'отмен']]
];
const installationColumns = [
  ['Нужно назначить', ['нужно назначить', 'не назначен']],
  ['Запланирован', ['заплан', 'назначен']],
  ['В работе', ['в работе', 'выезд', 'на объекте']],
  ['Выполнен', ['выполн', 'принят', 'закрыт']]
];
...
function ensureStyles() {
  if (document.getElementById('productionBoardV3Styles')) return;
  const style = document.createElement('style');
  style.id = 'productionBoardV3Styles';
  style.textContent = `
...
function isOverdue(value, status) {
  ...
  if (String(status || '').toLowerCase().includes('готов') || String(status || '').toLowerCase().includes('выполн')) return false;
...
function columnFor(status, columns) {
  const text = String(status || '').toLowerCase();
  return columns.find(([_, words]) => words.some((word) => text.includes(word)))?.[0] || columns[0][0];
}
...
async function loadOrderMap(orderIds) {
  ...
  await Promise.all(ids.map(async (id) => {
    const response = await supabaseClient
      .from('leader_orders')
      .select('id,order_number,project_name,status,deadline,layout_status,production_status,installation_status,progress_percent,client_total,contractor_cost,layout_link,data')
      .eq('id', id)
      .maybeSingle();
    if (!response.error && response.data) orders.set(response.data.id, response.data);
  }));
  return orders;
}
...
async function fetchData() {
  ...
  const orders = await loadOrderMap([...production.map((job) => job.order_id), ...installation.map((job) => job.order_id)]);
  state = { production, installation, orders };
  return state;
}

function matches(job, order, query) {
...
function renderCard(job, kind, order) {
  ...
  const actions = kind === 'production'
    ? `<button type="button" data-board-production-status="${esc(job.id)}" data-status="В работе">В работу</button><button type="button" class="v4-primary" data-board-production-status="${esc(job.id)}" data-status="Готово">Готово</button>`
    : `<button type="button" data-board-install-status="${esc(job.id)}" data-status="В работе">В работу</button><button type="button" class="v4-primary" data-board-install-status="${esc(job.id)}" data-status="Выполнен">Выполнен</button>`;
  return `<article class="v4-production-card ${overdue ? 'is-overdue' : ''}"><div class="v4-production-card-head"><b>${esc(job.title || order?.project_name || 'Задание')}</b><span class="v4-production-badge ${badgeClass(status)}">${esc(status || '—')}</span></div><div class="v4-production-meta"><span>Заказ: №${esc(order?.order_number || String(job.order_id || '').slice(0, 8))} — ${esc(order?.project_name || '—')}</span><span>${kind === 'production' ? 'Срок' : 'Монтаж'}: ${dt(date)}</span>${kind === 'production' ? `<span>Макет: ${esc(job.layout_status || order?.layout_status || '—')}</span><span>Себестоимость: ${money(job.contractor_cost || order?.contractor_cost)}</span>` : `<span>Адрес: ${esc(job.address || order?.data?.install_place || '—')}</span><span>Монтажник: ${esc(job.installer_name || '—')}</span>`}${overdue ? '<span style="color:#991b1b;font-weight:900">Просрочено</span>' : ''}</div>${file ? `<a class="v4-production-link" href="${esc(file)}" target="_blank" rel="noopener">Открыть файл / фото</a>` : ''}<div class="v4-production-card-actions"><button type="button" data-open-order="${esc(job.order_id)}">Открыть заказ</button>${actions}</div></article>`;
}

function renderColumns(items, kind, query, statusFilter) {
  const columns = kind === 'production' ? productionColumns : installationColumns;
  const groups = new Map(columns.map(([title]) => [title, []]));
  items.forEach((job) => {
    const order = state.orders.get(job.order_id);
    if (!matches(job, order, query)) return;
    const status = kind === 'production' ? job.production_status : job.install_status;
    if (statusFilter && status !== statusFilter) return;
    groups.get(columnFor(status, columns))?.push({ job, order });
  });
  return `<div class="v4-production-columns">${columns.map(([title]) => {
    const rows = groups.get(title) || [];
    return `<section class="v4-production-column"><h3>${esc(title)} <span>${rows.length}</span></h3>${rows.length ? rows.map(({ job, order }) => renderCard(job, kind, order)).join('') : '<div class="v4-production-empty">Нет заданий.</div>'}</section>`;
  }).join('')}</div>`;
}
...
function renderBoard(kind = 'production') {
  ...
  const productionOpen = state.production.filter((job) => !['Готово', 'Выдано', 'Отменено'].includes(job.production_status || '')).length;
  const installationOpen = state.installation.filter((job) => !['Выполнен', 'Закрыт', 'Отменён'].includes(job.install_status || '')).length;
  const overdue = [...state.production.map((job) => [job.deadline, job.production_status]), ...state.installation.map((job) => [job.scheduled_at, job.install_status])].filter(([date, status]) => isOverdue(date, status)).length;
  const statusOptions = [...new Set([...state.production.map((job) => job.production_status), ...state.installation.map((job) => job.install_status)].filter(Boolean))];
  box.innerHTML = `<div class="v4-production-board"><div class="v4-production-board-head"><div><p class="v4-kicker">Производственная доска</p><h2>Производство и монтаж</h2><p>Очередь заданий по всем заказам: макеты, производство, монтаж, сроки и быстрые статусы.</p></div><div class="v4-production-board-actions"><button type="button" class="v4-primary" data-v4-list-refresh="production">Обновить производство</button></div></div><div class="v4-production-board-summary"><div><span>Производственных</span><b>${state.production.length}</b></div><div><span>В работе / не передано</span><b>${productionOpen}</b></div><div><span>Монтажей</span><b>${state.installation.length}</b></div><div><span>Монтаж в работе</span><b>${installationOpen}</b></div><div><span>Просрочено</span><b>${overdue}</b></div></div><div class="v4-production-board-filters"><label>Поиск<input id="productionBoardSearch" type="search" value="${esc(query)}" placeholder="Заказ, статус, монтажник, адрес"></label><label>Статус<select id="productionBoardStatus"><option value="">Все статусы</option>${statusOptions.map((status) => `<option ${status === statusFilter ? 'selected' : ''}>${esc(status)}</option>`).join('')}</select></label><div><span class="v4-muted">Тип заданий</span><div class="v4-production-board-tabs"><button type="button" class="${kind === 'production' ? 'is-active' : ''}" data-production-board-kind="production">Производство</button><button type="button" class="${kind === 'installation' ? 'is-active' : ''}" data-production-board-kind="installation">Монтаж</button></div></div></div>${renderColumns(kind === 'production' ? state.production : state.installation, kind, query, statusFilter)}</div>`;
}
...
async function updateProductionStatus(jobId, status) {
  ...
  busy = true;
  try {
    const current = state.production.find((job) => job.id === jobId) || (await supabaseClient.from('leader_production_jobs').select('*').eq('id', jobId).single()).data;
    ...
    const orderResponse = await supabaseClient.from('leader_orders').update({ production_status: status, status: status === 'Готово' ? 'Готово' : 'В производстве', current_stage: `Производство: ${status}`, progress_percent: status === 'Готово' ? 85 : 60, updated_at: nowIso(), stage_updated_at: nowIso() }).eq('id', current.order_id);
    if (orderResponse.error) throw orderResponse.error;
    await safeInsert('leader_production_events', { job_id: jobId, order_id: current.order_id, event_type: 'Статус производства', old_status: current.production_status, new_status: status, body: `Изменено с производственной доски: ${status}`, created_by: v4State.user?.id || null, created_by_email: v4State.user?.email || null });
...
async function updateInstallStatus(jobId, status) {
  ...
  busy = true;
  try {
    const current = state.installation.find((job) => job.id === jobId) || (await supabaseClient.from('leader_installation_jobs').select('*').eq('id', jobId).single()).data;
    ...
    if (response.error) throw response.error;
    const orderResponse = await supabaseClient.from('leader_orders').update({ installation_status: status, current_stage: `Монтаж: ${status}`, progress_percent: status === 'Выполнен' ? 95 : 75, updated_at: nowIso(), stage_updated_at: nowIso() }).eq('id', current.order_id);
    if (orderResponse.error) throw orderResponse.error;
    await safeInsert('leader_installation_events', { job_id: jobId, order_id: current.order_id, event_type: 'Статус монтажа', old_status: current.install_status, new_status: status, body: `Изменено с производственной доски: ${status}`, created_by: v4State.user?.id || null });
    toast(`Монтаж: ${status}`);
    loaded = false;
    await loadProductionBoard(true);
  } catch (error) {
