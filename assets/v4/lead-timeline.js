...
function eventIcon(type, source) {
  ...
  if (value.includes('проблем')) return '!';
  return '●';
}

function eventTitle(event) {
  if (event.source === 'offer') return event.new_status ? `КП: ${event.new_status}` : (event.event_type || 'Событие КП');
...
function eventBody(event) { return event.body || event.comment || ''; }
function normalizeLeadEvent(event) { return { ...event, source: 'lead', sortDate: event.created_at || '' }; }
function normalizeOfferEvent(event) { return { ...event, source: 'offer', sortDate: event.created_at || '', body: event.comment || '' }; }
function mergedEvents() { return [...leadEvents.map(normalizeLeadEvent), ...offerEvents.map(normalizeOfferEvent)].sort((a, b) => new Date(b.sortDate) - new Date(a.sortDate)); }
  ...
  return `
    <article class="v4-timeline-item${sourceClass}">
      ...
      <div class="v4-timeline-content">
        <div class="v4-timeline-head"><b>${esc(eventTitle(event))}</b><span>${formatDateTime(event.created_at)}</span></div>
        ${body ? `<p>${esc(body)}</p>` : ''}
...
function render() {
  ...
  if (!lead?.id) { host.innerHTML = ''; return; }
  const events = mergedEvents();
  host.innerHTML = `
    <div class="v4-subcard-head"><div><h3>История и комментарии</h3><p>Фиксируйте звонки, сообщения в MAX, договорённости, причины пауз и важные изменения по заявке.</p></div><span class="v4-muted">Событий: ${events.length}</span></div>
    <form id="leadTimelineForm" class="v4-timeline-form">
      <div class="v4-form-grid">
        <label>Тип записи<select id="leadTimelineType"><option>Комментарий</option><option>Звонок</option><option>MAX</option><option>Статус</option><option>Следующий контакт</option><option>Проблема</option></select></label>
        <label>Быстрые заготовки<select id="leadTimelineTemplate"><option value="">Выбрать шаблон</option><option>Позвонил, клиент не ответил. Нужно повторить позже.</option><option>Связался в MAX, клиент уточняет детали.</option><option>Клиент попросил отправить коммерческое предложение.</option><option>Клиент думает, нужно вернуться к нему позже.</option><option>Клиенту дорого, нужно предложить альтернативный вариант.</option><option>Ждём от клиента макет / размеры / материалы.</option></select></label>
      </div>
      <label class="v4-timeline-body-label">Комментарий менеджера<textarea id="leadTimelineBody" rows="3" placeholder="Например: обсудили баннер 3×2, клиент ждёт КП в MAX, перезвонить завтра после 10:00"></textarea></label>
      <div class="v4-form-actions"><button id="leadTimelineSaveBtn" type="submit" class="v4-primary" ${saveBusy ? 'disabled' : ''}>Добавить запись</button><button id="leadTimelineReloadBtn" type="button" ${busy ? 'disabled' : ''}>Обновить историю</button></div>
    </form>
...
export async function loadLeadTimeline(leadId = v4State.currentLead?.id || v4State.route.leadId, options = {}) {
  ...
  if (!force && loadedLeadId === leadId && !busy) { scheduleRender(); return; }
  ...
  try {
    const [leadResponse, offerResponse] = await Promise.all([
      timeout(supabaseClient.from('leader_lead_events').select(LEAD_EVENT_FIELDS).eq('lead_id', leadId).order('created_at', { ascending: false }).limit(80), 12000, 'История заявки не загрузилась за 12 секунд'),
      timeout(supabaseClient.from('leader_commercial_offer_events').select(OFFER_EVENT_FIELDS).eq('lead_id', leadId).order('created_at', { ascending: false }).limit(40), 12000, 'История КП не загрузилась за 12 секунд')
    ]);
    ...
    if (currentLeadId === leadId) busy = false;
    render();
  }
}
...
export async function addLeadTimelineEvent({ leadId, eventType = 'Комментарий', body = '', oldStatus = null, newStatus = null } = {}) {
  ...
  const payload = { lead_id: targetLeadId, event_type: eventType, old_status: oldStatus, new_status: newStatus, body: cleanBody, created_by: v4State.user?.id || null, created_by_email: v4State.user?.email || null };
  const response = await timeout(supabaseClient.from('leader_lead_events').insert(payload).select(LEAD_EVENT_FIELDS).single(), 12000, 'Запись истории не сохранилась за 12 секунд');
  if (response.error) throw response.error;
  leadEvents = [response.data, ...leadEvents];
  loadedLeadId = targetLeadId;
  render();
...
async function saveForm() {
  ...
  if (!body) { toast('Напишите комментарий'); return; }
  saveBusy = true;
  render();
  try {
...
function bindEvents() {
  ...
  document.addEventListener('click', (event) => {
    if (event.target?.id === 'leadTimelineReloadBtn') loadLeadTimeline(undefined, { force: true });
  });
  document.addEventListener('change', (event) => {
    if (event.target?.id !== 'leadTimelineTemplate') return;
    const value = event.target.value || '';
    const textarea = byId('leadTimelineBody');
    if (textarea && value) { textarea.value = textarea.value ? `${textarea.value}\n${value}` : value; textarea.focus(); }
    event.target.value = '';
  });
  ...
  subscribeState((state) => {
    if (state.currentLead?.id && state.currentLead.id !== currentLeadId) loadLeadTimeline(state.currentLead.id);
    else scheduleRender();
  });
}
