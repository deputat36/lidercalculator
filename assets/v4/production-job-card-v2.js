...
async function fetchBundle(jobId) {
  ...
  const [itemsResponse, orderResponse, eventsResponse, installationResponse] = await Promise.all([
    ...
    job.order_id ? supabaseClient.from('leader_orders').select('id,order_number,project_name,status,deadline,layout_status,layout_link,production_status,installation_status,installation_address,progress_percent,production_comment,data,created_at').eq('id', job.order_id).single() : Promise.resolve({ data: null, error: null }),
