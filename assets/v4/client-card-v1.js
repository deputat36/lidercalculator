...
function ensureStyles() {
  if (document.getElementById('clientCardV1Styles')) return;
  const style = document.createElement('style');
  style.id = 'clientCardV1Styles';
  style.textContent = `
    .v4-client-modal{position:fixed;inset:0;z-index:690;background:rgba(15,23,42,.58);display:grid;place-items:center;padding:16px}
    .v4-client-modal-card{width:min(1040px,100%);max-height:92vh;overflow:auto;background:#fff;border:1px solid #dbeafe;border-radius:24px;box-shadow:0 28px 90px rgba(15,23,42,.35);padding:18px}
    .v4-client-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;border-bottom:1px solid #e2e8f0;padding-bottom:12px;margin-bottom:14px}
    .v4-client-head h2{margin:0}.v4-client-head p{margin:6px 0 0;color:#64748b}
    .v4-client-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin:12px 0}
    .v4-client-grid div{border:1px solid #e2e8f0;border-radius:16px;background:#f8fafc;padding:12px}
    .v4-client-grid span{display:block;color:#64748b;font-size:12px;font-weight:900;text-transform:uppercase}.v4-client-grid b{display:block;margin-top:5px;color:#0f172a}
    .v4-client-columns{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}.v4-client-section{border:1px solid #e2e8f0;border-radius:18px;padding:14px;background:#fff}.v4-client-section h3{margin:0 0 10px}
    .v4-client-item{border:1px solid #e2e8f0;border-radius:14px;padding:10px;margin:8px 0;background:#f8fafc}.v4-client-item-head{display:flex;justify-content:space-between;gap:10px}.v4-client-item-head b{overflow-wrap:anywhere}.v4-client-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
    .v4-client-badge{display:inline-flex;border-radius:999px;background:#e0f2fe;color:#075985;padding:4px 8px;font-size:12px;font-weight:900;white-space:nowrap}.v4-client-badge.is-good{background:#dcfce7;color:#166534}.v4-client-badge.is-warn{background:#fef3c7;color:#92400e}.v4-client-badge.is-danger{background:#fee2e2;color:#991b1b}
    .v4-client-empty{border:1px dashed #cbd5e1;border-radius:14px;padding:12px;color:#64748b;background:#f8fafc}.v4-client-close{white-space:nowrap}
...
function loading() {
  host().innerHTML = `<div class="v4-client-modal"><div class="v4-client-modal-card"><div class="v4-client-head"><div><h2>Карточка клиента</h2><p>Загружаю заявки, КП и заказы клиента...</p></div><button type="button" class="v4-client-close" data-client-card-close>Закрыть</button></div><div class="v4-client-empty">Загрузка...</div></div></div>`;
}
...
async function fetchLeadsForClient(baseLead) {
  let response = null;
  if (baseLead.phone) response = await supabaseClient.from('leader_leads').select('id,name,phone,source,service,status,budget,estimated_amount,message,city,created_at,next_contact_at,converted_client_id').eq('phone', baseLead.phone).order('created_at', { ascending: false }).limit(80);
  if (!response?.data?.length && baseLead.name) response = await supabaseClient.from('leader_leads').select('id,name,phone,source,service,status,budget,estimated_amount,message,city,created_at,next_contact_at,converted_client_id').eq('name', baseLead.name).order('created_at', { ascending: false }).limit(80);
  return response?.data || [baseLead];
}
async function fetchOrdersForClient(baseLead, leads) {
  ...
  for (const clientId of clientIds) {
    const response = await supabaseClient.from('leader_orders').select('id,order_number,lead_id,client_id,project_name,status,deadline,client_name,client_phone,client_total,payment_status,created_at').eq('client_id', clientId).order('created_at', { ascending: false }).limit(40);
    if (!response.error && response.data?.length) byClient.push(...response.data);
  }
  let byPhone = [];
  if (baseLead.phone) {
    const response = await supabaseClient.from('leader_orders').select('id,order_number,lead_id,client_id,project_name,status,deadline,client_name,client_phone,client_total,payment_status,created_at').eq('client_phone', baseLead.phone).order('created_at', { ascending: false }).limit(40);
    if (!response.error) byPhone = response.data || [];
  }
  const map = new Map();
  [...byClient, ...byPhone].forEach((order) => map.set(order.id, order));
...
async function fetchOffersForClient(leads) {
  ...
  for (const lead of leads.slice(0, 30)) {
    const response = await supabaseClient.from('leader_commercial_offers').select('id,lead_id,calculation_id,order_id,title,status,total_sum,valid_until,created_at').eq('lead_id', lead.id).order('created_at', { ascending: false }).limit(20);
    if (!response.error && response.data?.length) rows.push(...response.data);
  }
  const map = new Map();
  rows.forEach((offer) => map.set(offer.id, offer));
  return [...map.values()].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
}
