...
const ORDER_FIELDS = 'id,order_number,project_name,status,deadline,layout_status,layout_link,installation_address,production_comment,data';
const ITEM_FIELDS = 'id,name,unit,quantity,client_sum,comment,data,created_at';
...
async function fetchOrderBundle(orderId) {
  ...
  const itemsResponse = await supabaseClient.from('leader_order_items').select(ITEM_FIELDS).eq('order_id', orderId).order('created_at', { ascending: true }).limit(160);
  return { order, items: itemsResponse.error ? fallbackRows : (itemsResponse.data || fallbackRows) };
}

function rowName(row) { return row.name || row.title || 'Позиция'; }
function rowQty(row) { return row.quantity || row.qty || row.count || 1; }
function rowWidth(row) { return row.width || row.w || row.data?.width || ''; }
function rowHeight(row) { return row.height || row.h || row.data?.height || ''; }
