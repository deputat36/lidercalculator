...
const publicItems = (items) => (items || []).filter((item) => Number(item.client_sum || 0) > 0);
const taskText = (lead, calculation, offer) => {
  const lines = [];
  if (lead?.service) lines.push(lead.service);
  if (calculation?.title) lines.push(calculation.title);
  const full = String(offer.full_text || '').split('\n').map((line) => line.trim()).filter(Boolean);
  const idx = full.findIndex((line) => line.toLowerCase().includes('задача клиента'));
  if (idx >= 0 && full[idx + 1]) lines.push(full[idx + 1]);
  return [...new Set(lines)].slice(0, 3).join('. ') || 'Работы по согласованной заявке';
};

function mark() {
  return '<div class="mark"><i></i><i></i><i></i><i></i></div>';
}
function css() {
  return `:root{--o:#ff6a00;--d:#1a1a1a;--g:#24282d;--m:#7d8590;--l:#e6e8eb;--s:#f6f7f8}*{box-sizing:border-box}body{margin:0;background:#101214;color:var(--d);font-family:Montserrat,Arial,sans-serif;line-height:1.45}.page{width:210mm;min-height:297mm;margin:0 auto;background:#fff;position:relative;overflow:hidden;padding:0 14mm 14mm}.page:before{content:"";position:absolute;inset:0 0 auto;height:78mm;background:linear-gradient(135deg,rgba(255,255,255,.06) 0 10%,transparent 10% 34%,rgba(255,255,255,.05) 34% 48%,transparent 48%),linear-gradient(135deg,#0d0f12,#1a1a1a 72%,#101214);z-index:0}.page:after{content:"";position:absolute;right:-34mm;top:18mm;width:120mm;height:58mm;background:linear-gradient(135deg,transparent 0 47%,rgba(255,106,0,.95) 47% 60%,transparent 60%);transform:rotate(-7deg);opacity:.28;z-index:0}.top,.hero,.card,.items-wrap,.terms,.note,.sign,.foot{position:relative;z-index:1}.top{display:flex;justify-content:space-between;gap:18px;padding-top:14mm;color:#fff}.brand{display:flex;gap:13px;align-items:center}.brand h1{margin:0;font-size:29px;line-height:1;text-transform:uppercase;font-weight:900}.brand p{margin:6px 0 0;color:#e6e8eb;font-size:11px;letter-spacing:.42em}.mark{position:relative;width:48px;height:42px;flex:0 0 48px}.mark i{position:absolute;bottom:0;background:var(--o);transform:skewX(-17deg)}.mark i:nth-child(1){left:0;width:12px;height:27px}.mark i:nth-child(2){left:17px;width:14px;height:39px}.mark i:nth-child(3){left:30px;width:14px;height:42px;clip-path:polygon(0 26%,100% 0,82% 100%,0 100%)}.mark i:nth-child(4){right:0;width:16px;height:15px;background:#fff;clip-path:polygon(0 100%,55% 0,100% 100%);transform:none}.meta{text-align:right;color:#e6e8eb;font-size:12px}.meta b{display:block;color:#fff;font-size:20px;text-transform:uppercase}.hero{display:grid;grid-template-columns:1.12fr .88fr;gap:12px;margin:13mm 0 8mm}.panel,.card{border:1px solid var(--l);border-radius:20px;background:#fff;padding:14px;box-shadow:0 10px 25px rgba(26,26,26,.07)}.label{display:block;color:var(--m);font-size:10px;text-transform:uppercase;letter-spacing:.16em;font-weight:900;margin-bottom:5px}.big{font-size:30px;font-weight:900;text-transform:uppercase;line-height:1.05;letter-spacing:-.04em}.price{font-size:32px;color:var(--o);font-weight:900;line-height:1}.task{font-size:13px;color:#3f454b;margin:10px 0 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.field{background:var(--s);border-left:4px solid var(--o);border-radius:13px;padding:9px 10px}.items-wrap{margin-top:8mm}.title{display:flex;justify-content:space-between;align-items:center;margin:0 0 10px}.title h2{margin:0;font-size:16px;text-transform:uppercase}.title span{color:var(--m);font-size:11px}.items{width:100%;border-collapse:separate;border-spacing:0;font-size:12px;border-radius:14px;overflow:hidden}.items th{background:var(--d);color:#fff;text-align:left;padding:10px;text-transform:uppercase;font-size:10px;letter-spacing:.1em}.items td{padding:10px;border-bottom:1px solid var(--l);vertical-align:top}.items tr:nth-child(even) td{background:#f7f8f9}.num{text-align:right;white-space:nowrap}.total-row td{background:#fff4ec!important;font-weight:900}.total-row td:last-child{color:var(--o);font-size:18px}.terms{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:8mm}.term{border:1px solid var(--l);border-radius:16px;padding:12px;background:#fff}.term b{display:block}.term span{color:var(--m);font-size:12px}.note{margin-top:7mm;padding:13px;border-radius:16px;background:#1a1a1a;color:#fff;font-size:12px}.note b{color:var(--o)}.sign{display:grid;grid-template-columns:1fr 1fr;gap:14mm;margin-top:14mm}.sign div{border-top:1px solid var(--d);padding-top:5px;color:var(--m);font-size:12px}.foot{position:absolute;left:14mm;right:14mm;bottom:7mm;border-top:1px solid var(--l);padding-top:6px;color:var(--m);font-size:10px;display:flex;justify-content:space-between}.foot b{color:var(--d)}.presentation{padding:0 13mm 13mm}.presentation:before{height:112mm}.presentation .cover{position:relative;z-index:1;color:#fff;min-height:112mm;margin:0 -13mm 9mm;padding:14mm 13mm}.presentation .top{padding-top:0}.presentation .hero{display:block;margin:18mm 0 0;max-width:132mm}.presentation .big{font-size:38px;color:#fff}.presentation .task{color:#e6e8eb;max-width:112mm}.presentation .price-box{position:absolute;right:13mm;bottom:13mm;width:68mm;background:#fff;color:#1a1a1a;border-radius:22px;padding:14px}.presentation .price-box b{display:block;color:var(--o);font-size:31px}.presentation .after-cover{position:relative;z-index:1}.presentation .dark{background:#1a1a1a;color:#fff;border-color:#2d3339}.presentation .dark .label{color:#e6e8eb}.print-actions{position:fixed;right:16px;top:16px;display:flex;gap:8px;z-index:5}.print-actions button{border:0;border-radius:999px;background:var(--o);color:#fff;padding:10px 14px;font-weight:900;cursor:pointer}.print-actions button.secondary{background:var(--d)}@page{size:A4;margin:0}@media print{body{background:#fff}.page{margin:0}.print-actions{display:none}}`;
}
function rows(items) {
  const visible = publicItems(items);
  if (!visible.length) return '<tr><td colspan="5">Работы по согласованной заявке</td></tr>';
  return visible.map((item, i) => `<tr><td class="num">${i + 1}</td><td><b>${esc(item.name || 'Позиция')}</b>${item.comment ? `<br><span>${esc(item.comment)}</span>` : ''}</td><td class="num">${Number(item.qty || 0).toLocaleString('ru-RU')} ${esc(item.unit || 'шт')}</td><td class="num">${money(item.client_price)}</td><td class="num"><b>${money(item.client_sum)}</b></td></tr>`).join('');
}
...
async function loadBundle(offer) {
  ...
  if ((!lead || lead.id !== offer.lead_id) && offer.lead_id) {
    const leadRes = await timeout(supabaseClient.from('leader_leads').select(LEAD_FIELDS).eq('id', offer.lead_id).maybeSingle(), 12000, 'Заявка для печати не загрузилась за 12 секунд');
