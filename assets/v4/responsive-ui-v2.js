...
function renderOrderCard() {
  ...
  const comment = order.public_comment || order.production_comment || order.internal_comment || data.comment || data.production_comment || 'Комментарий не заполнен.';
...
async function openOrderCard(orderId) {
  ...
  renderOrderCard();
  try {
    const response = await supabaseClient.from('leader_orders').select('id,order_number,project_name,status,deadline,client_name,client_phone,client_total,contractor_cost,profit,payment_status,layout_status,production_status,installation_status,production_comment,internal_comment,public_comment,data,lead_id').eq('id', orderId).maybeSingle();
    if (response.error) throw response.error;
