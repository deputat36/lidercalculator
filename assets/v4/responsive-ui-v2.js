...
const SECTIONS = [
  ...
  { key: 'orders', label: 'Заказы', id: 'ordersListSection', title: 'Заказы', description: 'Общий список заказов: статус, срок, клиент, сумма и оплата.' },
...
function errorBox(key, message) {
  return `<div class="v4-empty is-error">Ошибка загрузки раздела: ${esc(message)}<div class="v4-form-actions" style="margin-top:12px"><button type="button" class="v4-primary" data-v4-list-refresh="${esc(key)}">Повторить загрузку</button></div></div>`;
}
async function guardedLoad(key, loader, force = false) {
  ...
  state.loading[key] = Promise.resolve()
    ...
    .catch((error) => {
      ...
      state.errors[key] = friendlyError(error);
      setContent(key, errorBox(key, state.errors[key]));
    })
    .finally(() => { state.loading[key] = null; });
  return state.loading[key];
}
async function loadOrders(force = false) {
  return guardedLoad('orders', async () => {
    const response = await timeout(supabaseClient.from('leader_orders').select('id,order_number,project_name,status,deadline,client_name,client_phone,client_total,payment_status,created_at,layout_status,data').order('created_at', { ascending: false }).limit(100), 30000, 'Заказы не загрузились за 30 секунд');
...
async function loadCalculations(force = false) {
  return guardedLoad('calculations', async () => {
    const response = await timeout(supabaseClient.from('leader_lead_calculations').select('id,lead_id,title,status,client_total,contractor_cost,profit,margin_percent,order_id,created_at').order('created_at', { ascending: false }).limit(100), 30000, 'Расчёты не загрузились за 30 секунд');
    if (response.error) throw response.error;
    const rows = response.data || [];
    const total = rows.reduce((sum, row) => sum + Number(row.client_total || 0), 0);
...
async function loadClients(force = false) {
  return guardedLoad('clients', async () => {
    const response = await timeout(supabaseClient.from('leader_leads').select('id,name,phone,city,source,service,status,budget,estimated_amount,created_at,next_contact_at').order('created_at', { ascending: false }).limit(300), 30000, 'Клиенты не загрузились за 30 секунд');
    if (response.error) throw response.error;
    const map = new Map();
...
function renderOrderCard() {
  ...
  if (state.orderError) { host.innerHTML = errorBox('orders', state.orderError); return; }
...
async function openOrderCard(orderId) {
  ...
  renderOrderCard();
  try {
    const response = await timeout(supabaseClient.from('leader_orders').select('*').eq('id', orderId).maybeSingle(), 30000, 'Карточка заказа не загрузилась за 30 секунд');
    if (response.error) throw response.error;
