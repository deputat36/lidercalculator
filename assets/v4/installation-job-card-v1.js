...
function ensureStyles() {
  ...
  s.id = 'installationJobCardV1Styles';
  s.textContent = `
...
function host() {
  let el = document.getElementById('installationJobCardV1');
  if (!el) { el = document.createElement('div'); el.id = 'installationJobCardV1'; document.body.appendChild(el); }
  return el;
}
...
async function saveJob(id) {
  ...
  try {
    ...
    await Promise.all([
      supabaseClient.from('leader_installation_events').insert({ job_id: id, order_id: old.order_id, event_type: 'Обновление монтажа', old_status: old.install_status, new_status: status, body: 'Монтажное задание обновлено из карточки монтажа', created_by: v4State.user?.id || null }),
    ...
    toast('Монтаж сохранён'); setStatus('Монтажное задание сохранено', 'good'); await openCard(id);
...
async function addComment(id) {
  ...
  try { const r = await supabaseClient.from('leader_installation_comments').insert({ job_id: id, comment_type: 'internal', body, created_by: v4State.user?.id || null }); if (r.error) throw r.error; toast('Комментарий добавлен'); await openCard(id); } catch (e) { toast(friendlyError(e)); } finally { busy = false; }
}
...
async function printJob(id) { try { const b = bundle?.job?.id === id ? bundle : await fetchBundle(id); const w = window.open('', '_blank', 'noopener,noreferrer'); if (!w) throw new Error('Браузер заблокировал окно печати'); w.document.open(); w.document.write(printHtml(b)); w.document.close(); } catch (e) { toast(friendlyError(e)); } }
function enhance() {
