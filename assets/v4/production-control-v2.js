...
const PRODUCTION_FIELDS = 'id,order_id,title,production_status,deadline,layout_status,file_url,contractor_cost,created_at,updated_at';
const INSTALLATION_FIELDS = 'id,order_id,title,install_status,scheduled_at,address,installer_name,installer_phone,created_at,updated_at';
...
function render() {
  ...
  const g = groups();
  const overdueCount = g.prodOverdue.length + g.instOverdue.length;
  const soonCount = g.prodSoon.length + g.instSoon.length;
  const problemCount = g.prodProblems.length + g.instProblems.length;
...
async function loadData(force = false) {
  ...
  const [orders, production, installation] = await Promise.all([
    safeQuery('Заказы', supabaseClient.from('leader_orders').select(ORDER_FIELDS).order('created_at', { ascending: false }).limit(60)),
    safeQuery('Производственные задания', supabaseClient.from('leader_production_jobs').select(PRODUCTION_FIELDS).order('deadline', { ascending: true }).limit(60)),
    safeQuery('Монтажные задания', supabaseClient.from('leader_installation_jobs').select(INSTALLATION_FIELDS).order('scheduled_at', { ascending: true }).limit(60))
  ]);
  state = { orders, production, installation };
  loaded = true;
  busy = false;
...
function boot() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (tab) { event.preventDefault(); event.stopPropagation(); showProductionControl(); return; }
