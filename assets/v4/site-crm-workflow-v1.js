...
function ensureStyles() {
  ...
  style.textContent = `
    .v4-site-crm-panel{border:1px solid #fde68a;background:#fffbeb;border-radius:18px;padding:12px;margin:12px 0;color:#5a3b00}.v4-site-crm-panel h3{margin:0 0 6px;color:#111827}.v4-site-crm-panel p{margin:0 0 10px;font-weight:800}.v4-site-crm-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px}.v4-site-crm-stat{border:1px solid #fef3c7;background:#fff;border-radius:14px;padding:10px}.v4-site-crm-stat span{display:block;font-size:12px;text-transform:uppercase;font-weight:900;color:#92400e}.v4-site-crm-stat b{display:block;font-size:22px;color:#111827}.v4-site-crm-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.v4-site-crm-actions button{background:#fff}.v4-site-crm-actions .v4-primary{background:#f6c343;border-color:#f6c343;color:#111827}.v4-lead-card.is-site-lead{border-color:#f6c343!important;background:linear-gradient(180deg,#fff,#fffbeb)!important}.v4-lead-card .v4-site-lead-label{display:inline-flex;margin-left:6px;border-radius:999px;background:#f6c343;color:#111827;padding:3px 8px;font-size:11px;font-weight:900}.v4-lead-card .v4-site-lead-page{display:block;margin-top:6px;color:#92400e;font-weight:800;font-size:12px;overflow-wrap:anywhere}@media(max-width:720px){.v4-site-crm-actions{display:grid}.v4-site-crm-actions button{width:100%}}
  `;
...
function applySiteCardLabels() {
  document.querySelectorAll('.v4-lead-card[data-id]').forEach((card) => {
    ...
    if (title && !title.querySelector('.v4-site-lead-label')) title.insertAdjacentHTML('beforeend', '<span class="v4-site-lead-label">сайт</span>');
...
function renderPanel() {
  ...
  const s = siteStats();
  panel.innerHTML = `<h3>Сайт ↔ CRM</h3><p>Быстрый контроль заявок с сайта: что пришло, что новое и где не назначен следующий контакт.</p><div class="v4-site-crm-grid"><div class="v4-site-crm-stat"><span>Всего заявок</span><b>${s.all}</b></div><div class="v4-site-crm-stat"><span>С сайта</span><b>${s.site}</b></div><div class="v4-site-crm-stat"><span>Новые с сайта</span><b>${s.siteNew}</b></div><div class="v4-site-crm-stat"><span>Без контакта</span><b>${s.siteNoNext}</b></div></div><div class="v4-site-crm-actions"><button type="button" class="v4-primary" data-site-crm-filter="site">Показать заявки с сайта</button><button type="button" data-site-crm-filter="new">Новые с сайта</button><button type="button" data-site-crm-filter="no-next">Без следующего контакта</button><button type="button" data-site-crm-filter="reset">Сбросить фильтр</button><button type="button" data-site-crm-refresh>Обновить заявки</button></div>`;
  applySiteCardLabels();
}
...
function applyFilter(type) {
  ...
  setTimeout(() => {
    ...
    if (type === 'no-next') {
      document.querySelectorAll('.v4-lead-card[data-id]').forEach((card) => {
        const lead = (v4State.leads || []).find((item) => String(item.id) === String(card.dataset.id));
        if (lead && hasNextContact(lead)) card.style.display = 'none';
      });
    }
  }, 60);
}
