...
function unpaid(order) { const t = String(order.payment_status || '').toLowerCase(); return !t || t.includes('не') || t.includes('част') || t.includes('долг') || t.includes('ожид'); }
function noLayout(order) { const t = String(order.layout_status || '').toLowerCase(); return !t || t.includes('нет') || t.includes('нуж') || t.includes('ожид') || t.includes('не готов'); }
function productionStatus(order) { const t = String(order.status || '').toLowerCase(); return t.includes('производ') || t.includes('работ') || t.includes('печать') || t.includes('монтаж'); }
function grouped() {
  const activeRows = rows.filter(active);
  return {
    active: activeRows,
    overdue: activeRows.filter((o) => { const d = daysUntil(o.deadline); return d !== null && d < 0; }),
    today: activeRows.filter((o) => daysUntil(o.deadline) === 0),
    next3: activeRows.filter((o) => { const d = daysUntil(o.deadline); return d !== null && d > 0 && d <= 3; }),
...
function ensureSection() {
  ...
  section.hidden = true;
  section.innerHTML = `<div class="v4-section-head"><div><h2>Контроль заказов</h2><p>Сводка по заказам: сроки, оплата, макеты, производство и срочные задачи.</p></div><button type="button" class="v4-primary" data-order-control-refresh>Обновить заказы</button></div><div id="orderControlContent"><div class="v4-empty">Раздел загрузится при открытии.</div></div>`;
...
function ensureNav() {
  ...
  button.type = 'button'; button.dataset.v4TabButton = 'order_control'; button.textContent = 'Контроль заказов';
  if (anchor) anchor.insertAdjacentElement('afterend', button); else nav.appendChild(button);
}
function stat(label, value, type = '') { return `<div class="v4-order-stat ${type}"><span>${esc(label)}</span><b>${esc(value)}</b></div>`; }
...
function card(order, note = '') {
  ...
  const cls = d !== null && d < 0 ? 'is-danger' : d !== null && d <= 3 ? 'is-warn' : '';
  const deadline = d === null ? 'срок не указан' : d < 0 ? `просрочен на ${Math.abs(d)} дн.` : d === 0 ? 'сегодня' : `через ${d} дн.`;
  return `<article class="v4-order-item ${cls}"><div class="v4-order-item-head"><h4>${esc(title(order))}</h4><small>${esc(order.status || 'Новый')}</small></div><small>${esc(order.client_name || 'Клиент не указан')} · ${esc(order.client_phone || 'телефон не указан')}</small><small>Срок: ${dateRu(order.deadline)} (${esc(deadline)})</small><small>Оплата: ${esc(order.payment_status || 'Не указана')} · Сумма: ${money(order.client_total)}</small><small>Макет: ${esc(order.layout_status || 'Не указан')}</small>${note ? `<small>${esc(note)}</small>` : ''}<button type="button" data-open-order="${esc(order.id)}">Открыть заказ</button></article>`;
}
function top(list, mapper) { return list.slice(0, 6).map(mapper).join('') || '<div class="v4-empty">Нет заказов в этой группе.</div>'; }
function render() {
  ensureSection();
  const content = document.getElementById('orderControlContent'); if (!content) return;
  if (busy) { content.innerHTML = '<div class="v4-empty">Загружаю заказы...</div>'; return; }
  if (!loaded) { content.innerHTML = '<div class="v4-empty">Нажмите «Обновить заказы» или откройте раздел ещё раз.</div>'; return; }
  const g = grouped();
  const activeTotal = g.active.reduce((sum, o) => sum + Number(o.client_total || 0), 0);
  const warnHtml = warnings.length ? `<div class="v4-order-warnings">${warnings.map(esc).join('; ')}. Раздел показан в частичном режиме.</div>` : '';
  content.innerHTML = `${warnHtml}<div class="v4-order-control-grid">${stat('Всего заказов', rows.length)}${stat('Активные', g.active.length, g.active.length ? 'is-good' : '')}${stat('Просрочены', g.overdue.length, g.overdue.length ? 'is-danger' : '')}${stat('Срок сегодня', g.today.length, g.today.length ? 'is-warn' : '')}${stat('Срок 1–3 дня', g.next3.length, g.next3.length ? 'is-warn' : '')}${stat('Не оплачено / частично', g.unpaid.length, g.unpaid.length ? 'is-danger' : '')}${stat('Макет не готов', g.noLayout.length, g.noLayout.length ? 'is-warn' : '')}${stat('Активная сумма', money(activeTotal))}</div><div class="v4-order-actions"><button type="button" class="v4-primary" data-order-tab-open>Открыть все заказы</button><button type="button" data-order-control-refresh>Обновить</button></div><div class="v4-order-columns"><section class="v4-order-column"><h3>Просроченные сроки</h3><div class="v4-order-list">${top(g.overdue, (o) => card(o, 'Нужно срочно проверить производство / выдачу'))}</div></section><section class="v4-order-column"><h3>Срок сегодня и 1–3 дня</h3><div class="v4-order-list">${top([...g.today, ...g.next3], (o) => card(o, 'Ближайший срок'))}</div></section><section class="v4-order-column"><h3>Оплата под контролем</h3><div class="v4-order-list">${top(g.unpaid, (o) => card(o, 'Проверьте оплату'))}</div></section><section class="v4-order-column"><h3>Макеты и производство</h3><div class="v4-order-list">${top([...g.noLayout, ...g.production], (o) => card(o, noLayout(o) ? 'Макет не готов или требует проверки' : 'Заказ в производственном статусе'))}</div></section></div>`;
}
async function loadOrders(force = false) {
  ...
  try {
    ...
    const response = await timeout(supabaseClient.from('leader_orders').select(ORDER_FIELDS).order('created_at', { ascending: false }).limit(80), 18000, 'Контроль заказов: долгий ответ Supabase');
...
function showOrderControl() {
  ensureSection(); ensureNav(); document.body.dataset.v4Tab = 'order_control';
  document.querySelectorAll('[data-v4-tab-button]').forEach((b) => b.classList.toggle('is-active', b.dataset.v4TabButton === 'order_control'));
  document.querySelectorAll('[data-v4-managed-section]').forEach((s) => { s.hidden = s.dataset.v4ManagedSection !== 'order_control'; });
  loadOrders(false); window.scrollTo({ top: 0, behavior: 'smooth' });
}
function boot() {
  ensureSection(); ensureNav();
  document.addEventListener('leader-v4:crm-ready', () => { setTimeout(ensureNav, 300); if (document.body.dataset.v4Tab === 'order_control') loadOrders(false); });
  document.addEventListener('leader-v4:tab-opened', (e) => { setTimeout(ensureNav, 150); if (e.detail?.tab === 'order_control' || document.body.dataset.v4Tab === 'order_control') loadOrders(false); });
  document.addEventListener('click', (event) => {
    const tab = event.target.closest?.('[data-v4-tab-button="order_control"]');
    if (tab) { event.preventDefault(); event.stopPropagation(); showOrderControl(); return; }
    if (event.target.closest?.('[data-order-control-refresh]')) { event.preventDefault(); loadOrders(true); return; }
