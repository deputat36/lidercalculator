...
const ORDER_FIELDS = 'id,order_number,project_name,status,deadline,layout_status,layout_link,installation_address,production_comment,data,created_at';
const ITEM_FIELDS = 'id,name,unit,quantity,comment,data,created_at';
...
function firstValue(...values) { return values.map((value) => String(value ?? '').trim()).find(Boolean) || ''; }
function getLayoutLink(order) { const data = dataOf(order); return firstValue(order.layout_link, data.layout_link, data.design_link, data.cloud_link, data.maked_link, data.layoutUrl, data.designUrl); }
function getPlacePhotoLink(order) { const data = dataOf(order); return firstValue(data.place_photo_link, data.install_photo_link, data.placePhotoLink, data.installPhotoLink, data.photo_link, data.photos_link); }
function getInstallPlace(order) { const data = dataOf(order); return firstValue(order.installation_address, data.install_place, data.mount_address, data.install_address, data.placement, data.location); }
function getProductionComment(order) { const data = dataOf(order); return firstValue(order.production_comment, data.production_comment, data.technical_task, data.production_task, data.comment); }
function getRows(order, items = []) {
  if (items.length) return items;
  const data = dataOf(order);
  const candidates = [data.rows, data.items, data.order_rows, data.orderItems];
  for (const candidate of candidates) if (Array.isArray(candidate) && candidate.length) return candidate;
  return [];
}
function rowComment(row) { return firstValue(row.production_comment, row.technical_comment, row.comment, row.note, row.data?.comment); }
function buildRows(rows) {
  if (!rows.length) return '<tr><td colspan="4">Позиции не найдены в данных заказа. Проверьте связанный расчёт или КП.</td></tr>';
...
function printHtml(bundle) {
  ...
  const data = dataOf(order);
  const rows = getRows(order, items);
...
async function loadOrder(orderId) {
  ...
  if (!orderResponse.data) throw new Error('Заказ не найден');
  return { order: orderResponse.data, items: itemsResponse.error ? [] : itemsResponse.data || [] };
}
...
window.addEventListener('click', (event) => {
  ...
  if (!orderId) { alert('Не удалось определить заказ. Вернитесь в список заказов и откройте заказ заново.'); return; }
