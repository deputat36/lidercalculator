...
function host() {
  let element = document.getElementById('productionJobCardV1');
  if (!element) {
    element = document.createElement('div');
    element.id = 'productionJobCardV1';
...
function loading() {
  host().innerHTML = `<div class="v4-job-modal"><div class="v4-job-card"><div class="v4-job-head"><div><h2>Производственное задание</h2><p>Загружаю данные...</p></div><button type="button" data-production-job-close>Закрыть</button></div><div class="v4-job-empty">Загрузка...</div></div></div>`;
}
function errorBox(text) {
  host().innerHTML = `<div class="v4-job-modal"><div class="v4-job-card"><div class="v4-job-head"><div><h2>Производственное задание</h2><p>Ошибка загрузки</p></div><button type="button" data-production-job-close>Закрыть</button></div><div class="v4-job-empty">${esc(text)}</div></div></div>`;
}
async function fetchBundle(jobId) {
  ...
  const [itemsResponse, orderResponse, eventsResponse, installationResponse] = await Promise.all([
    supabaseClient.from('leader_production_job_items').select('*').eq('job_id', jobId).order('created_at', { ascending: true }),
    job.order_id ? supabaseClient.from('leader_orders').select('id,order_number,project_name,status,deadline,layout_status,layout_link,production_status,installation_status,installation_address,progress_percent,data,created_at').eq('id', job.order_id).single() : Promise.resolve({ data: null, error: null }),
    supabaseClient.from('leader_production_events').select('*').eq('job_id', jobId).order('created_at', { ascending: false }).limit(40),
    supabaseClient.from('leader_installation_jobs').select('id,title,install_status,address,scheduled_at,installer_name,installer_phone').eq('production_job_id', jobId).order('created_at', { ascending: false }).limit(20)
  ]);
  ...
  return {
    ...
    installation: installationResponse.error ? [] : installationResponse.data || []
  };
}
...
async function saveJob(jobId) {
  ...
  try {
    ...
    const patch = {
      title: value('jobEditTitle') || old.title,
      production_status: status,
      layout_status: value('jobEditLayout') || old.layout_status,
      priority: value('jobEditPriority') || old.priority,
      deadline: deadlineRaw ? new Date(deadlineRaw).toISOString() : null,
      file_url: value('jobEditFile') || null,
      technical_task: value('jobEditTask') || null,
      contractor_comment: value('jobEditContractorComment') || null,
      internal_comment: value('jobEditInternalComment') || null,
      updated_at: nowIso()
    };
    ...
    await Promise.all([
      supabaseClient.from('leader_production_events').insert({ job_id: jobId, order_id: old.order_id, event_type: 'Обновление задания', old_status: old.production_status, new_status: status, body: 'Производственное задание обновлено из карточки задания', created_by: v4State.user?.id || null, created_by_email: v4State.user?.email || null }),
      old.order_id ? supabaseClient.from('leader_orders').update({ production_status: status, layout_status: patch.layout_status, layout_link: patch.file_url, current_stage: `Производство: ${status}`, updated_at: nowIso(), stage_updated_at: nowIso() }).eq('id', old.order_id) : Promise.resolve()
    ]);
    toast('Производственное задание сохранено');
    setStatus('Производственное задание сохранено', 'good');
    await openJobCard(jobId);
  } catch (error) {
