...
function printStyles() {
  return `:root{--blue:#1d4ed8;--dark:#0f172a;--muted:#64748b;--line:#dbe3ef;--soft:#f8fafc;--green:#15803d;}*{box-sizing:border-box}body{margin:0;background:#e5e7eb;color:var(--dark);font-family:Arial,"Helvetica Neue",sans-serif;line-height:1.45}.page{width:210mm;min-height:297mm;margin:0 auto;background:#fff;padding:16mm 15mm 14mm;position:relative;overflow:hidden}.page:before{content:"";position:absolute;inset:0 0 auto 0;height:9mm;background:linear-gradient(90deg,#0f172a,#1d4ed8,#38bdf8)}.top{display:grid;grid-template-columns:1fr auto;gap:18px;align-items:start;margin-top:9mm;padding-bottom:10mm;border-bottom:2px solid var(--line)}.brand{display:flex;gap:12px;align-items:center}.logo{width:48px;height:48px;border-radius:16px;background:linear-gradient(135deg,#1d4ed8,#38bdf8);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:22px;letter-spacing:-1px}.brand h1{margin:0;font-size:25px;line-height:1.05;letter-spacing:-.4px}.brand p{margin:4px 0 0;color:var(--muted);font-size:12px}.doc-meta{text-align:right;font-size:12px;color:var(--muted)}.doc-meta b{display:block;color:var(--dark);font-size:18px;margin-bottom:4px}.hero{display:grid;grid-template-columns:1.25fr .75fr;gap:12px;margin:10mm 0}.panel{border:1px solid var(--line);border-radius:18px;padding:14px;background:#fff}.panel.soft{background:linear-gradient(135deg,#eff6ff,#fff)}.label{display:block;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:900;margin-bottom:4px}.big{font-size:30px;font-weight:900;color:var(--blue);line-height:1.05}.client-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}.client-grid div{border-top:1px dashed var(--line);padding-top:8px}h2{font-size:16px;margin:0 0 9px}.task{font-size:14px}.items{width:100%;border-collapse:collapse;margin-top:6px;font-size:12px}.items th{background:#0f172a;color:#fff;text-align:left;padding:9px;border:1px solid #0f172a}.items td{padding:9px;border:1px solid var(--line);vertical-align:top}.items tr:nth-child(even) td{background:#f8fafc}.items .num{text-align:right;white-space:nowrap}.items .total-row td{background:#eff6ff!important;font-weight:900;font-size:14px}.terms{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:10mm}.term{border:1px solid var(--line);border-radius:16px;padding:12px;background:#fff}.term b{display:block;margin-bottom:4px}.term span{color:var(--muted);font-size:12px}.note{margin-top:8mm;padding:12px;border-radius:16px;background:#f0fdf4;border:1px solid #bbf7d0;color:#14532d;font-size:12px}.sign{display:grid;grid-template-columns:1fr 1fr;gap:14mm;margin-top:15mm}.sign div{border-top:1px solid var(--dark);padding-top:5px;color:var(--muted);font-size:12px}.footer{position:absolute;left:15mm;right:15mm;bottom:8mm;border-top:1px solid var(--line);padding-top:5px;color:var(--muted);font-size:10px;display:flex;justify-content:space-between;gap:12px}.print-actions{position:fixed;right:16px;top:16px;display:flex;gap:8px;z-index:5}.print-actions button{border:0;border-radius:999px;background:#1d4ed8;color:#fff;padding:10px 14px;font-weight:900;cursor:pointer}.print-actions button.secondary{background:#0f172a}@page{size:A4;margin:0}@media print{body{background:#fff}.page{margin:0;box-shadow:none}.print-actions{display:none}.footer{position:absolute}}`;
}
...
function buildPrintHtml({ offer, calculation, items, lead }) {
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(offer.title || 'Коммерческое предложение')}</title><style>${printStyles()}</style></head><body><div class="print-actions"><button onclick="window.print()">Печать / сохранить PDF</button><button class="secondary" onclick="window.close()">Закрыть</button></div><main class="page"><header class="top"><div class="brand"><div class="logo">Л</div><div><h1>РА «Лидер»</h1><p>Рекламное производство · Борисоглебск</p></div></div><div class="doc-meta"><b>Коммерческое предложение</b><div>Дата: ${esc(today)}</div><div>Действует до: ${esc(dateRu(offer.valid_until))}</div><div>№ ${esc(String(offer.id || '').slice(0, 8).toUpperCase())}</div></div></header><section class="hero"><div class="panel soft"><span class="label">Предложение</span><div class="big">${esc(offer.title || calculation?.title || 'Работы по заявке')}</div><p class="task">${esc(task)}</p></div><div class="panel"><span class="label">Итоговая стоимость</span><div class="big">${money(total)}</div><p>Стоимость указана для согласованного состава работ.</p></div></section><section class="panel"><h2>Клиент</h2><div class="client-grid"><div><span class="label">Имя</span><b>${esc(lead?.name || 'Не указано')}</b></div><div><span class="label">Телефон</span><b>${esc(lead?.phone || 'Не указано')}</b></div><div><span class="label">Город</span><b>${esc(lead?.city || 'Борисоглебск')}</b></div><div><span class="label">Услуга</span><b>${esc(lead?.service || calculation?.title || 'Рекламные работы')}</b></div></div></section><section class="panel" style="margin-top:8mm"><h2>Состав предложения</h2><table class="items"><thead><tr><th>№</th><th>Наименование</th><th>Кол-во</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>${itemRows(items)}<tr class="total-row"><td colspan="4">Итого к оплате</td><td class="num">${money(total)}</td></tr></tbody></table></section><section class="terms"><div class="term"><b>Запуск в работу</b><span>После подтверждения состава, стоимости и внесения предоплаты.</span></div><div class="term"><b>Макет</b><span>Производство запускается после согласования финального макета.</span></div><div class="term"><b>Сроки</b><span>Срок зависит от готовности макета, материалов и загрузки производства.</span></div><div class="term"><b>Доставка / монтаж</b><span>Условия доставки и монтажа согласуются отдельно, если они не включены в состав.</span></div></section>${publicComment ? `<div class="note"><b>Примечание:</b> ${esc(publicComment)}</div>` : '<div class="note"><b>Важно:</b> предложение действительно при сохранении указанных параметров заказа, материалов и объёма работ.</div>'}<section class="sign"><div>Представитель РА «Лидер»</div><div>Клиент / согласовано</div></section><footer class="footer"><span>РА «Лидер» · рекламное производство</span><span>Коммерческое предложение сформировано в CRM</span></footer></main></body></html>`;
}
