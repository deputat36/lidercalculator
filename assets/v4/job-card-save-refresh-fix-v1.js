...
async function saveProduction(jobId) {
  ...
  try {
    ...
    const oldResponse = await supabaseClient.from('leader_production_jobs').select('*').eq('id', jobId).single();
...
async function saveInstallation(jobId) {
  ...
  try {
    ...
    const oldResponse = await supabaseClient.from('leader_installation_jobs').select('*').eq('id', jobId).single();
